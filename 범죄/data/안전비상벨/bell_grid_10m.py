...
grid_counts = gdf.groupby(['grid_x', 'grid_y']).size().reset_index(name='bell_count')
grid_counts['center_lon'] = (grid_counts['grid_x'] + 0.5) * grid_size
grid_counts['center_lat'] = (grid_counts['grid_y'] + 0.5) * grid_size

output_path = 'bell_grid_10m.csv'
grid_counts.to_csv(output_path, index=False)
logger.info(f"격자별 비상벨 개수 저장 완료: {output_path}")

logger.info("\n=== 데이터 확인 ===")
logger.info(f"총 격자 수: {len(grid_counts)}")
logger.info(f"비상벨이 있는 격자 수: {len(grid_counts[grid_counts['bell_count'] > 0])}")
logger.info("\n비상벨 개수 분포:")
logger.info(grid_counts['bell_count'].describe())

m = folium.Map(location=[37.5665, 126.9780], zoom_start=11)
heat_data = [[row['center_lat'], row['center_lon'], row['bell_count']] 
             for _, row in grid_counts[grid_counts['bell_count'] > 0].iterrows()]
HeatMap(heat_data, radius=15, blur=10).add_to(m)
map_path = 'bell_grid_10m_map.html'
m.save(map_path)
