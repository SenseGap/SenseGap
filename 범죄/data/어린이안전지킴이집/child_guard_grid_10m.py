...
# 2. 아동안전지킴이집 데이터 불러오기 후 GeoDataFrame 변환
# (예시: df = pd.read_csv(...))
# gdf = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df['lon'], df['lat']), crs='EPSG:4326')

# 1. 아동안전지킴이집 데이터 불러오기
child_guard_path = '아동안전지킴이집.csv'
df = smart_read_csv(child_guard_path)
...
grid_counts = gdf.groupby(['grid_x', 'grid_y']).size().reset_index(name='child_guard_count')
grid_counts['center_lon'] = (grid_counts['grid_x'] + 0.5) * grid_size
grid_counts['center_lat'] = (grid_counts['grid_y'] + 0.5) * grid_size

output_path = 'child_guard_grid_10m.csv'
grid_counts.to_csv(output_path, index=False)
logger.info(f"격자별 아동안전지킴이집 개수 저장 완료: {output_path}")

logger.info("\n=== 데이터 확인 ===")
logger.info(f"총 격자 수: {len(grid_counts)}")
logger.info(f"아동안전지킴이집이 있는 격자 수: {len(grid_counts[grid_counts['child_guard_count'] > 0])}")
logger.info("\n아동안전지킴이집 개수 분포:")
logger.info(grid_counts['child_guard_count'].describe())

m = folium.Map(location=[37.5665, 126.9780], zoom_start=11)
heat_data = [[row['center_lat'], row['center_lon'], row['child_guard_count']] 
             for _, row in grid_counts[grid_counts['child_guard_count'] > 0].iterrows()]
HeatMap(heat_data, radius=15, blur=10).add_to(m)
map_path = 'child_guard_grid_10m_map.html'
m.save(map_path)
